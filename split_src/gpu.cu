...
{
	struct myWorkload * workloadObject = (struct myWorkload *) data;
	workloadObject->check_sum[workloadObject->id] = 1;

	printf("Checksum: ");
	for (int i = 0; i < NUMNODES; i++){
		printf("%d ", workloadObject->check_sum[i]);
	}
	printf("\n");
...
extern "C" void launch_master(int *d_arr, int *check_sum, int num_nodes)
{
	...
	myWorkload * workloadObject = (struct myWorkload *) malloc(sizeof(struct myWorkload));

	cudaStream_t streams[num_nodes];

	workloadObject->check_sum = check_sum;
	...
	for (int i = 0; i < num_nodes; i++)
	{
		...
		workloadObject->id = i;
		response = cudaStreamAddCallback(streams[i], myStreamCallback, workloadObject, 0);
		if(response != cudaSuccess){
